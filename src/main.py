...
def read_series_from_txt(filename: str) -> np.array:
    ...
    Args:
        filename (str): name of .txt data file to import, located in /data folder
...
def estimate_wavelength(timeseries: np.array) -> int:
    """Uses the scipy.signal.periodogram to find strongest wavelength of timeseries,
...
def extract_periodicity(timeseries: np.array) -> np.array:
    """Uses estimate_wavelength to generate periodic features by optimising location of
    the period windows, via minimising the average pairwise L2 dist of the periodic
    features.
    ...
    Returns:
        np.array: 2D array whose rows are the exclusive periodic subsets of the timeseries.
    """
    ...
    # For each location of the period windows, find the average pairwise L2 dist of the subsets
    # of the timeseries.
